...
INSERT INTO todo_history_NEW (id, uuid, todo, created_by, created_at, property, old_value, new_value)
SELECT 
    id, 
    uuid, 
    (SELECT id FROM todo WHERE uuid = todo_history.todo_uuid),
    (SELECT id FROM subgraph_user WHERE uuid = todo_history.created_by), 
    created_at, 
    property, 
    old_value, 
    new_value
